...
const loadVariables = async (client, basePath, filePath, config, border) => {
  ...
  const output = [
    [colors[config?.title || 'white'] + "Variable Name" + colors.end, colors[config?.title || 'white'] + "Type" + colors.end, colors[config?.title || 'white'] + "Table" + colors.end]
  ]
  ...
  client.variables(v.variables, v.table || 'main')
    ...
    console.error('Variables not provided! Skipping the loader initializatin.')
    return
  }
